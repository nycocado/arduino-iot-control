...
{
    char* array_commands[] = {
        "Identidade Válida  ", "Identidade Inválida"
    };                         // Event name labels
    ...
    printf(
        "  \\_____\\___/|_| |_|\\__|_|  \\___/|_|\\___| |_____|\\____/ |_|   \n"
    );
    printf("\n");
    ...
    tty_fd =
        open(argv[1], O_RDWR | O_NONBLOCK); // Opens communication with Arduino
    ...
    {
        if (read(tty_fd, &c, 1) > 0 && c != '0' && c != '1' && c != '2' &&
            c != '3') // If Arduino sends a character, write to terminal (skip
                      // ID chars)
        {
        ...
        if (read(STDIN_FILENO, &c, 1) >
            0) // If user types something, send to Arduino
        {
            write(tty_fd, &c, 1); // Writes character to Arduino
        }

        if (c == '0') // If Arduino sends '0', add "Valid Identity" event with
                      // timestamp
        {
            Event event =
                malloc(sizeof(struct Event_)); // Allocates event memory
        ...
        if (c == '1') // If Arduino sends '1', add "Invalid Identity" event with
                      // timestamp
        {
            Event event =
                malloc(sizeof(struct Event_)); // Allocates event memory
        ...
        if (c == '2') // If Arduino sends '2', show event table
        {
            ...
            {
                write(
                    STDOUT_FILENO, "Nenhum evento registrado\n\r", 27
            ...
            else // If list has events, show them
            {
                ...
                write(STDOUT_FILENO, "\n\r", 2); // Skips a line
                write(
                    STDOUT_FILENO, "Lista de Eventos:\n\r", 19
                ); // Writes event list header
                for (int i = 0; i < list_size(list);
                     i++) // For each event, show name and timestamp
                {
                    Event event =
                        list_iterator_get_next(list); // Gets next list element
                    write(
                        STDOUT_FILENO, event->name, strlen(event->name)
                    );                              // Writes event name
                    write(STDOUT_FILENO, " - ", 3); // Writes separator
                    write(
                        STDOUT_FILENO,
                        ctime(&event->time),
                        strlen(ctime(&event->time))
                    ); // Writes event timestamp
                    write(
                        STDOUT_FILENO, "\r ", 1
